'use strict';

function makeExecutable(){
    var FS = require("fs");
    var Path = require("path");
    var PlatformArchPath = Path.join(process.platform, process.platform === "win32" ? '7za.exe' : '7za');
    var filePath = Path.join(__dirname, "..","binaries", PlatformArchPath);
    if (process.platform !== "win32") {
        FS.chmodSync(filePath, 757);
    } 
    return;
}

var Zip = function () {};

Zip.prototype.add = require('./add');
Zip.prototype.delete = require('./delete');
Zip.prototype.extract = require('./extract');
Zip.prototype.extractFull = require('./extractFull');
Zip.prototype.list = require('./list');
Zip.prototype.test = require('./test');
Zip.prototype.update = require('./update');

module.exports = Zip;
makeExecutable();
