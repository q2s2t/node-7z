...
/**
 * Regexp to detect STAGE_HEADERS info vs. files
 */
const infoHeadersRegex = /^(?<property>.+) = (?<value>.+)$/

/**
 * Regexp to detect STAGE_FOOTER info vs. files
 */
const infoFooterRegex = /^(?<property>.+): (?<value>.+)$/
...
export function onSubprocessData (stream, chunk) {
  ...
  lines.forEach(function (line) {
    ...
    if (stream._stage === STAGE_HEADERS) {
      const matchInfo = line.match(infoHeadersRegex)
      if (matchInfo) {
    ...
    if (stream._stage === STAGE_FOOTER) {
      const matchInfo = line.match(infoFooterRegex)
      if (matchInfo) {
        stream.info[matchInfo.groups.property] = matchInfo.groups.value
      }
      return
